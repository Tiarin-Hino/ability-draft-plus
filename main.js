console.log("---- Electron/Node.js Version Info ----");
console.log("Electron Version:", process.versions.electron);
console.log("Node.js Version:", process.versions.node);
console.log("N-API Version:", process.versions.napi); // This is the crucial one
...
const { getAbilityWinrates } = require('./src/database/queries'); // Import for winrate fetching
...
const { processDraftScreen } = require('./src/imageProcessor'); // TFJS version

// --- Paths and URLs ---
const dbPath = path.join(__dirname, 'dota_ad_data.db');
const coordinatesPath = path.join(__dirname, 'config', 'layout_coordinates.json'); // Corrected path
const TARGET_RESOLUTION = "2560x1440"; // Ensure this matches your config and testing
...
app.whenReady().then(() => {
  try {
    setupDatabase(dbPath); // No need to keep db instance open from setup
...
function sendStatusToRenderer(event, message) {
  if (event && event.sender && !event.sender.isDestroyed()) {
    event.sender.send('scrape-status', message);
  }
}

// --- IPC Listeners for Scraping (Unchanged) ---
...
ipcMain.on('scan-draft-screen', async (event) => {
  console.log('Received scan-draft-screen request.');
  const startTime = performance.now();
  sendStatusToRenderer(event, 'Processing screen with ML model...');

  try {
    // 1. Get raw predicted names from imageProcessor (TFJS version)
    // Note: `iconsDir` and `dbPath` are no longer passed to processDraftScreen
    const rawResults = await processDraftScreen(coordinatesPath, TARGET_RESOLUTION);

    const { ultimates: predictedUltimates, standard: predictedStandard } = rawResults;

    // 2. Fetch Winrates for the identified names
    ...
    if (allIdentifiedNames.length > 0) {
      try {
        winrateMap = getAbilityWinrates(dbPath, allIdentifiedNames); // Call imported function
        ...
        // Proceed with an empty map; results will show N/A for winrates
      }
    } else {
      console.log('No abilities identified by ML model to fetch winrates for.');
    }

    // 3. Format results with names and winrates
    const formatResultsWithWinrates = (namesArray, wrMap) => {
      return namesArray.map(name => ({
        name: name || 'Unknown', // Keep 'Unknown' for null predictions
    ...
    console.log(`Screen processing and winrate fetching finished in ${durationMs} ms.`);
    ...
    console.error('Error during scan-draft-screen processing:', error);
